...
export function getPlaylistWebSocketUrl(
  playlistId: number,
  token: string,
  deviceId?: string,
): string {
  ...
  // Note: The OpenAPI spec defines this path as /ws/playlists/playlists/{playlist_id}
  apiUrl.pathname =
    apiUrl.pathname.replace(/\/?api\/v1\/?$/, "") +
    `/ws/playlists/${playlistId}`;

  apiUrl.searchParams.set("token", token);
  if (deviceId) {
    apiUrl.searchParams.set("device_id", deviceId);
  }
